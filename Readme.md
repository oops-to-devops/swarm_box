...
# https://gilt.readthedocs.io/en/latest/
  - git: https://github.com/oops-to-devops/swarm-box.git
    version: master
    dst: deployment/provisioners/swarm-box/
    post_commands:
      - make
```
...
```
      ...
      config.vm.provision "ansible" do |ansible|
          ansible.playbook = "provisioners/swarm-box/playbook.yml"
          ansible.galaxy_role_file = "provisioners/swarm-box/requirements.yml"
          ansible.galaxy_roles_path = "deployment/provisioners/swarm-box/roles"
...
resource "null_resource" "baseboxed_servers" {
  ...
  provisioner "local-exec" {
    command = "$INFRASTRUCTURE_ROOT_DIR/provisioners/swarm-box/provision_box.sh"
